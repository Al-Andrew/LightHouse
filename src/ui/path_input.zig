//! A small owned single-line editor. Used for directory locations; its contents
//! are never submitted to the shell, including when pasted.
...
pub const PathInput = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, initial: []const u8) !PathInput {
        var self: PathInput = .{ .allocator = allocator };
        try self.buffer.appendSlice(allocator, initial);
        self.cursor = initial.len;
        return self;
    }
    pub fn deinit(self: *PathInput) void {
        self.buffer.deinit(self.allocator);
    }
    pub fn text(self: *const PathInput) []const u8 {
        return self.buffer.items;
    }

    /// Paint one editable row, scrolling whole glyphs to keep the caret visible.
    pub fn paint(self: *const PathInput, painter: ui.Painter, style: ui.Style) !void {
        painter.fill(style);
    ...
    fn insert(self: *PathInput, bytes: []const u8) !void {
        _ = self.clearSelected();
        // Keep directory entry bounded; ignore pasted newlines, NUL and other
    ...
    pub fn event(self: *PathInput, ev: *const input.Event) !Result {
        switch (ev.kind) {
            .paste_start => {
                self.pasting = true;
                return .editing;
...
test "path input replaces selection, edits Unicode, and cannot submit pasted newlines" {
    var editor = try PathInput.init(std.testing.allocator, "/old");
    defer editor.deinit();
...
test "scrolled editor keeps its caret and wide glyphs inside a nested field" {
    var editor = try PathInput.init(std.testing.allocator, "/long/界é");
    defer editor.deinit();
