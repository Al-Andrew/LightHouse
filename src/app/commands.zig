...
pub const descriptions = [_]Description{
    ...
    .{ .id = .path, .label = "Path", .help = "Path", .bindings = &.{.{ .byte = input.control('l'), .text = "Ctrl+L" }} },
    .{ .id = .filter, .label = "Filter", .help = "Filter: Enter browse; Esc close; Ctrl+U clear", .bindings = &.{.{ .byte = '/', .text = "/" }} },
